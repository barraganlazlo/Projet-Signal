...
private :
  ...
  long unsigned int data_nb;         // Nombre de données
...
public:
  ...
  void getData8(unsigned char** data, // pointeur sur le tableau de donées lorsque l'on est sur des données 8 bits
                long int* size);           // pointeur sur la taille du tableau
  ...
  void read(char* fileName, bool verbose=false);   //Lecture d'un fichier wave
  void write(char* fileName, bool verbose=false);
  int getFreqEch() const;
  Wave();
  ~Wave();
