...
void Wave::getData8(unsigned char** data, // Tableau de donées lorsque l'on est sur des données 8 bits
                    long int* size) {          // Taille du tableau
...
void Wave::read(char* fileName, bool verbose) {
  ...
  // TEST D'OUVERTURE
  if (fd == NULL) {
    if(verbose) cout<<"Wave::read: Erreur, le fichier "<<fileName<<" n'est pas ouvrable\n";
    exit(-1);
  }
  // TEST DE LECTURE
  if (fread (header, 1, 44, fd) != 44) {
    if(verbose) cout<<"Wave::read: Erreur, impossible de lire dans le fichier "<<fileName<<" les 44 octets d'entete\n";
    exit(-1);
  }
  ...
  if (!((file_type[0] == 'R') && (file_type[1] == 'I') && (file_type[2] == 'F') && (file_type[3] == 'F'))) {
    if(verbose) cout<<"Wave::read: Erreur, le fichier "<<fileName<<" n'est pas un fichier RIFF\n";
    exit(-1);
  }
  ...
  if(verbose) cout<<"Wave::read: Le nombre d'octets restant à lire est : "<<file_size<<endl;
  ...
  if (!((file_id[0] == 'W') && (file_id[1] == 'A') && (file_id[2] == 'V') && (file_id[3] == 'E'))) {
    if(verbose) cout<<"Wave::read: Erreur, le fichier "<<fileName<<" n'est pas un fichier WAVE\n";
    exit(-1);
  }
  ...
  if (!((chunk_id[0] == 'f') && (chunk_id[1] == 'm') && (chunk_id[2] == 't') /*&& (chunk_id[3] == ' ')*/)) {
    if(verbose) cout<<"Wave::read: Erreur, le fichier "<<fileName<<" n'est pas un fichier 'fmt'\n";
    exit(-1);
  }
  ...
  if(verbose) cout<<"Wave::read: Le nombre de canaux est : "<<channels_nb<<endl;
  ...
  if(verbose) cout<<"Wave::read: La fréquence d'échantillonge est : "<<sampling_freq<<endl;
  ...
  if(verbose) cout<<"Wave::read: Le nombre d'octets par seconde est : "<<bytes_per_second<<endl;
  ...
  if(verbose) cout<<"Wave::read: Le nombre d'octets par échantillon est : "<<bytes_per_sample<<endl;
  if (bytes_per_sample != (bytes_per_second/sampling_freq)) {
    if(verbose) cout<<"Wave::read: bytes_per_sample != (bytes_per_second/sampling_freq)\n";
    exit(-1);
  }
  ...
  if(verbose) cout<<"Wave::read: Le nombre de bits par donnée est : "<<depth<<endl;
  if (depth != (8*bytes_per_sample/channels_nb)) {
    if(verbose) cout<<"Wave::read: depth != (8*bytes_per_sample/channels_nb)\n";
    exit(-1);
  }
  ...
  for (unsigned int i=0; i<4; i++, pos++) {
    ...
    if(verbose) cout<<data_id[i];
  }
  if(verbose) cout<<endl;
  if (!((data_id[0] == 'd') && (data_id[1] == 'a') && (data_id[2] == 't') && (data_id[3] == 'a'))) {
    if(verbose) cout<<"Wave::read: Erreur, le fichier "<<fileName<<" est corrompu (pas de chaine 'data' dans le header)\n";
    exit(-1);
  }
  ...
  if(verbose) cout<<"Wave::read: Le nombre d'octets restant à lire est : "<<data_size<<endl;
  if (data_size != (file_size-36)) {
    if(verbose) cout<<"Wave::read: data_size != (file_size-36)\n";
    exit(-1);
  }
  ...
  switch (depth) {
    ...
    case 8:
      ...
      if (fread (data8, 1, data_nb, fd) != data_nb) { //Les donnees sont sur 8 bits
        if(verbose) cout<<"Wave::read: Erreur, impossible de lire dans le fichier "<<fileName<<" le bon nombre d'octet\n";
        exit(-1);
      }
    ...
    default:
      if(verbose) cout<<"Wave::read: Erreur, la profondeur (depth = "<<depth<<") est inconnue\n";
      exit(-1);
  }
...
int Wave::getFreqEch() const{
  return this->sampling_freq;
}

void Wave::write(char* fileName, bool verbose) {

  if(verbose) cout <<"Wave::write(char* fileName)\n";
  ...
  // TEST D'OUVERTURE
  if (fd == NULL) {
    if(verbose) cout<<"Wave::write: Erreur, le fichier "<<fileName<<" n'est pas ouvrable\n";
    exit(-1);
  }
  ...
  // file_size
  if(verbose) cout<<"file_size = "<< file_size <<endl;
  for (unsigned int i=0; i<4; i++, pos++) {
    //Bit de poids faible en premier
...
//    cout<<"header[pos] = "<<(int)header[pos];
  ...
  if (!((header[pos-4] == 'f') && (header[pos-3] == 'm') && (header[pos-2] == 't') /*&& (chunk_id[3] == ' ')*/)) {
    if(verbose) cout<<header[pos-4]<<header[pos-3]<<header[pos-2]<<header[pos-1]<<endl;
    if(verbose) cout<<"Wave::write: Erreur, le header est mal remplit pour le chunk_id 'fmt'\n";
    exit(-1);
  }
  ...
  if (fwrite (header, 1, 44, fd) != 44) {
    if(verbose) cout<<"Wave::write: Erreur, impossible d'écrire dans le fichier "<<fileName<<" le header\n";
    exit(-1);
  }

  if(verbose) cout<<"Fin écriture du header\n";
  //LECTURE DES DONNEES
  switch (depth) {
    case 8:
      if(verbose) cout<<"fwrite (data8, \n";
      if (fwrite (data8, 1, data_nb, fd) != data_nb) { //Les donnees sont sur 8 bits
        if(verbose) cout<<"Wave::write: Erreur, impossible d'écrire dans le fichier "<<fileName<<" le bon nombre d'octet\n";
        exit(-1);
      }
      break;
    case 16:
      ...
      break;
    default:
      if(verbose) cout<<"Wave::write: Erreur, la profondeur (depth = "<<depth<<") est inconnue\n";
      exit(-1);
  }

  if(verbose) cout<<"Fin écriture des datas\n";
...
//    exit(-1);
//  }
////  /*TEST*/for (unsigned int i=0; i<24; i++) {
////  /*TEST*/  ifs>>c;
////  /*TEST*/}
////
